...
const sheetVariants = cva(
  "fixed z-50 gap-4 border-border bg-card p-6 shadow-soft transition ease-in-out duration-300",
  {
    variants: {
      side: {
        ...
        right: "inset-y-0 right-0 h-full w-[90%] border-l sm:max-w-md",
      },
    },
    defaultVariants: {
      side: "right",
  ...
  ({ side = "right", className, children, ...props }, ref) => (
    <SheetPortal>
      ...
      <SheetPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
        {children}
        <SheetClose className="absolute right-4 top-4 rounded-sm text-muted-foreground transition-opacity hover:opacity-80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
          <X className="h-4 w-4" />
